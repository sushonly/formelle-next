...
export async function generateMetadata({ params }: { params: Promise<{ identity: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://www.formellewear.com/wardrobe/${identity.id}`,
    },
  }
}

export default async function IdentityPage({ params }: { params: Promise<{ identity: string }> }) {
  const { identity: identityId } = await params
  const [identity, looks, allIdentities] = await Promise.all([
    getIdentity(identityId),
    getLooksByIdentity(identityId),
    getIdentities(),
  ])
