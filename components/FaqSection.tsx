  ...
  return (
    <section id="faq" style={{ background: 'var(--parchment)', padding: '72px 56px' }}>
      <div style={{ maxWidth: '760px', margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(36px,4vw,56px)', fontWeight: 300, lineHeight: 1.1, letterSpacing: '-0.02em', marginBottom: '40px' }}>
          Good to <em style={{ fontStyle: 'italic', color: 'var(--accent)' }}>know</em>
        </h2>
        <div>
          {FAQS.map((f, i) => (
            <div key={i} style={{ borderBottom: '0.5px solid rgba(17,17,17,0.1)' }}>
              <div onClick={() => setOpenIdx(openIdx === i ? null : i)} style={{ padding: '22px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', fontFamily: 'var(--font-display)', fontSize: '19px', color: 'var(--noir)' }}>
                {f.q}
